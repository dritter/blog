...
install and export from `ctrl+P` and find the options to export. Make sure that the default app for HTML is the browser for automatic printing option.
...
install mingw version > 4.8.1 (tested on 4.9.2), download here
...
`ctrl+b` to compile, `ctrl+shift+b` to run the code.
...
Ctrl+Alt+F: Format current file.
CMD+K, CMD+F: Format current selection.
